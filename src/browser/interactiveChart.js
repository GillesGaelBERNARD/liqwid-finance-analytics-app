...
function chartRenderComparison(container, state) {
  ...
  const lines = summaries.map(item => state.valueMode === "flow"
    ...
    : state.valueMode === "ratio"
      ...
      : `<p><strong style="color:${chartSafeColor(item.color)}">${chartEscape(item.label)}</strong> · ${chartEscape(chartFormatValue(state, item.start))} → ${chartEscape(chartFormatValue(state, item.end))} · change ${chartEscape(chartSignedValue(state, item.delta))}${Number.isFinite(item.percent) ? ` (${item.percent >= 0 ? "+" : ""}${item.percent.toFixed(1)}%)` : ""}</p>`
  ).join("");
