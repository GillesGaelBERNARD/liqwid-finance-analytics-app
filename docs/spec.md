...
12. The complete refreshed dataset is staged in memory. Only after every required API surface and every derived analysis succeeds are immutable raw captures, canonical clean CSV tables, metadata, and the two append-only loan-observation tables committed to the selected folder or archive and all views redrawn. Deterministic analysis outputs are rebuilt in memory and are not persisted. A cancelled picker makes no API request; a failed refresh leaves the last good local data and display intact.
...
The folder or archive keeps `raw/api`, `clean`, `computed`, and `metadata` separated. Raw official GraphQL captures use compact JSON only where preserving response structure, request variables, and fetch metadata is necessary. Canonical clean tables, metadata, and the two append-only observation histories use CSV. A dataset that can be deterministically recreated from a canonical saved table is not persisted as another file. Portable ZIP saves use standard DEFLATE where supported and retain compatibility with earlier uncompressed archives.
...
- Revenue: official protocol/holder allocation and run-rate context. Statistics sharing a period are grouped under one prominent period label instead of repeating that period on every card.
...
- Debt repayments: daily flow, a colored 7/30/90-day debt-coverage matrix, fast-versus-slow repayment intensity, and dry spells.
- Interest: interest formation and repayment, gaps, and a colored 7/30/90-day interest-coverage matrix.
- Selected-market debt and interest top statistics show each current-valued cumulative gap in USD together with the native cumulative gap and the exact latest implied asset price used for that valuation.
- Revenue: monthly market fee-paying activity from reported interest repayments and origination fees, with official retained DAO revenue explicitly unavailable at market level.
...
Data status is not an analytics scope or section. Its access sits beside **Open another data archive**, **Save data**, and **Fetch new data**. The visible overlay contains four compact evidence groups: Data coverage, Current loan population, Consistency checks, and Known coverage boundaries. It uses direct values such as markets covered, missing days, complete periods, position counts, represented borrow share, and reconciliation differences rather than an opaque integrity score. Loan-row reconciliation is split into two checks: coverage below 99.5% is undercoverage (official borrow not represented by returned loan rows), while coverage above 100.5% is overcoverage (the official loan-detail and market-aggregate API surfaces may not have refreshed to the same snapshot). Both checks accept the inclusive 99.5% to 100.5% range. Undercoverage is the more concerning failed state and uses the red exclamation mark; overcoverage is a partial synchronization boundary and uses the quieter orange tilde. The symbols are derived from each check's current status, not assigned as static presentation. The same inclusive band controls whether analytics show a nearby undercoverage or overcoverage warning; exact percentages, differences, and per-row reconciliation labels remain visible. The saved-observation value counts the union of protocol-level participation and health observation timestamps, so it matches every observation represented by the loan-history graphs even when an older archive contains only one of the two computed rows at a timestamp. Zero missing liquidation days and zero reconciliation failures are a successful state and never use alert styling. Population-bar segments use the same single color as their legend marker. The collapsed technical audit expands into four useful groups: archive provenance, dataset inventory, validation evidence, and validation rules. Opening an archive, completing a first full-history fetch, and completing an incremental fetch each rebuild Data status from the newly active dataset before rendering it.
...
- Debt accrued: inferred new debt flow, calculated independently for each market in its native asset as `current borrow - prior borrow + debt repaid`, floored at zero. Repayment is added back before inferred formation is compared with reported repayment, so repayment is not silently netted and then subtracted a second time. The first observation is unavailable. This is derived from official fields because the API exposes no direct debt-accrued flow.
- Daily debt-flow gap: inferred native debt accrued minus native debt repaid. Rolling and cumulative debt-flow gaps are accumulated in native units first. Their USD values are then calculated at each observation from that market's current native-to-USD price. A borrow and equal later repayment therefore close the cumulative gap even when the asset price changes between the two events.
...
- Protocol debt-flow gaps: the sum of each market's already calculated USD gap for the same observation and window. Native quantities from different assets are never added together.
- Interest accrued USD: daily interest generated by outstanding debt.
- Interest repaid USD: daily interest paid back.
- Interest net gap: native interest accrued minus native interest repaid for each market. Positive values mean accrued interest outpaced repayments for that day.
- Cumulative interest-flow gap: cumulative native interest accrued minus cumulative native interest repaid, valued in USD at the market's current observed price. It indicates repayment lag in asset units and is not a separate balance outside current borrow.
- Protocol interest-flow gaps: the sum of current USD gap values calculated independently for every market. The protocol gap is never calculated by subtracting historical `interestRepaidInUsd` from historical `interestAccruedInUsd`, because those flows may have been converted at different asset prices.
...
- Official LQ-staker allocation: `analytics.fees.dailyHoldersRevenue`, kept separate from DAO revenue and reconciled to the holder/staker interest and origination fields in `breakdown`.
...
- Protocol cumulative interest-flow gap and rolling net interest gap through time.
...
- Daily official allocation from `analytics.fees`, stacked as DAO interest, DAO origination fees, LQ-staker interest, and LQ-staker origination fees.
- Monthly official allocation aggregated from the canonical daily rows with the same four recipient/source components. The current partial month remains visible.
- Historical annualized DAO/treasury run rate, derived from rolling windows of 90 consecutive complete official daily rows and multiplied by `365.25 / 90`, through the latest closed UTC day.
...
- Market fee-paying activity from reported interest repayment and origination-fee flows, with explicit zero/low-activity interpretation. The app does not estimate retained protocol or DAO revenue by market because the official API does not expose the historical recipient allocation at that level.
